...
void ReadInTraces(const char *trace_prefix, int start_file_no, int end_file_no) {
    ...
    for(int datafileCnt = start_file_no; datafileCnt <= end_file_no; ++datafileCnt) {
        ...
        while(fread(buffer, 1, 16, fin) == 16) {
            ...
            // 跳过前8字节时间戳，拷贝后8字节（srcIP + dstIP）
            memcpy(tmp_five_tuple.key, buffer + 8, 8); 
...
public:
    ArraySampler(size_t memory_bytes, size_t total_flows)
        : total_inserts(0), collisions(0), rng(random_device{}()) {
        
        // 计算桶数量：内存(位) / (64位键 + 32位计数器)
        bucket_size = (memory_bytes * 8) / (64 + 32); // 64位 = 8字节
        if (bucket_size == 0) bucket_size = 1;
        
        // 计算采样率：桶数量 / 总流数
        sample_rate = static_cast<double>(bucket_size) / total_flows;
        ...
        // 使用高质量的随机数生成器初始化BOBHash32
        uniform_int_distribution<int> seed_dist(0, 1228); // 0到1228的均匀分布
        int seed = seed_dist(rng); // 从rng获取随机种子
        bob_hash = new BOBHash32(seed); // 使用随机种子初始化哈希函数
    }
    ~ArraySampler() {
        delete bob_hash;
    }
    ...
    size_t hash_function(const FIVE_TUPLE& key) const {
        return bob_hash->run(key.key, 8) % bucket_size; // 改为8字节
    }
    
    void insert(const FIVE_TUPLE& key) override {
        // 采样决策
        uniform_real_distribution<double> dist(0.0, 1.0);
        if (dist(rng) > sample_rate) return;
        ...
        do {
            ...
            if (memcmp(buckets[index].key.key, key.key, 8) == 0) { // 改为8字节比较
                buckets[index].count++;
                return;
            }
    ...
    int query(const FIVE_TUPLE& key) override {
        ...
        do {
            if (buckets[index].occupied && 
                memcmp(buckets[index].key.key, key.key, 8) == 0) { // 改为8字节比较
    ...
    void get_distribution(vector<double>& dist) override {
        ...
        for (const auto& pair : freq_count) {
            dist[pair.first] = static_cast<double>(pair.second) / total_sampled;
        }
    }
    
    size_t memory_usage() const override {
        // 每个桶：64位键 + 32位计数器 = 96位 = 12字节
        return bucket_size * 12; // 从17字节改为12字节
    }
...
private:
    struct Hash {
        ...
        size_t operator()(const FIVE_TUPLE& key) const {
            return bob_hash->run(key.key, 8); // 改为8字节
        }
    };
    
    struct Equal {
        bool operator()(const FIVE_TUPLE& a, const FIVE_TUPLE& b) const {
            return memcmp(a.key, b.key, 8) == 0; // 改为8字节比较
        }
    };
...
public:
    HashMapSampler(size_t memory_bytes, size_t total_flows)
        : total_inserts(0), rng(random_device{}()) {
        
        // 计算最大元素数：内存(位) / (64位键 + 32位计数器 + 200位开销)
        size_t max_entries = (memory_bytes * 8) / (64 + 32 + 200); // 64位 = 8字节
        
        // 计算采样率：最大元素数 / 总流数
        sample_rate = static_cast<double>(max_entries) / total_flows;
        if (sample_rate > 1.0) sample_rate = 1.0;
    ...
    size_t memory_usage() const override {
        // 估算内存使用：每个条目大约 64位键 + 32位计数器 + 200位开销 = 296位 = 37字节
        return hash_map.size() * 37; // 从42字节改为37字节
    }
...
void real_distribution(const vector<FIVE_TUPLE>& trace, vector<double>& dist) {
    ...
    dist.resize(max_freq + 1, 0.0);
    for (const auto& pair : freq_count) {
        dist[pair.first] = static_cast<double>(pair.second) / freq_map.size();
    }
}
...
// 修改后的ARE计算函数
double calculate_mrd(const vector<FIVE_TUPLE>& trace, BaseSampler& sampler) {
    ...
    // 计算估计频率和误差
    for (const auto& pair : real_freq) {
        FIVE_TUPLE key;
        memcpy(key.key, pair.first.c_str(), 8); // 改为8字节
        
        int est_freq = sampler.query(key);
        int real_freq_val = pair.second;
        
        // 使用算术平均作为分母
        double denominator = (real_freq_val/2.0 + est_freq/2.0);
        
        if (denominator > 0) {  // 避免除零错误
            double error = abs(real_freq_val - est_freq) / denominator;
            total_error += error;
            count++;
        }
        // 如果分母为0（即real_freq_val和est_freq都为0），跳过该项
    }
...
void evaluate_sampler(BaseSampler& sampler, const vector<FIVE_TUPLE>& trace, 
    ...
    // 计算ARE
    double are = calculate_mrd(trace, sampler);
    ...
    auto insert_duration = chrono::duration_cast<chrono::milliseconds>(insert_end - start);
    auto dist_duration = chrono::duration_cast<chrono::milliseconds>(dist_end - insert_end);
    ...
    if (csv_file.is_open()) {
        ...
        csv_file << insert_duration.count() << endl;       // Insert time (ms)
    }
}
...
int main(int argc, char* argv[]) {
    ...
    if (!options.output_csv.empty()) {
        ...
        // 写入CSV表头
        csv_file << "packets number,memory,method,WMRD,ARE,Insert time\n";
    }
    cout << endl;

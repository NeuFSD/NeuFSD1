...
// 修改数据包大小为21字节，提取前13字节
#define DEFAULT_COUNTERS  16384    // 2^14哈希表大小
#define PACKET_SIZE       21       // 每个数据包21字节（原16字节）
#define EXTRACT_SIZE      13       // 提取前13字节
...
ProcessingResult process_dat(const char* path, uint32_t seed) {
    ...
    while (1) {
        ...
        for (size_t i = 0; i < packets; ++i) {
            ...
            // 修改哈希输入为前13字节（原pkt+8取8字节）
            const uint32_t hash = MurmurHash3_x86_32(pkt, EXTRACT_SIZE, seed);

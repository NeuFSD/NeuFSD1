...
PRETRAINED_ROOT = '0_pretrained_weights/ViT_1_10_results_1e-2'
FINETUNED_OUT_ROOT = '0_finetuned_results/ViT_1_10_results_1e-2' # 👈 新增这一行：独立的输出根目录
LABEL_DIR_NAME = '1_10_chazhi'
...
def read_one_type_da(root_dir_name, dataset_id, label_dir):
    ...
    data_tensor = torch.reshape(data_tensor, (data_tensor.shape[0], 3, 64, 64))
...
if __name__ == "__main__":
    ...
    for idx, (filename, is_train) in enumerate(task_pbar):
        ...
        if is_train:
            ...
            print(f"\n" + "="*50)
            print(f"🔥 开始 Finetune (联合训练块，起点: {dataset_id})")
            
            train_dt_list, train_lt_list = [], []
            train_names = []
            ...
            for j in range(idx, len(task_list)):
                ...
                if dt_train is not None:
                    train_dt_list.append(dt_train)
                    train_lt_list.append(lt_train)
                    train_names.append(next_dataset_id)
                processed_train_files.add(next_filename)

            if not train_dt_list:
                continue
            ...
            for j in range(val_start_idx, len(task_list)):
                ...
                if dt_val is not None:
                    val_dt_list.append(dt_val)
                    val_lt_list.append(lt_val)
                    val_names.append(next_dataset_id)
            
            val_loaders = [] 
            if val_names:
                print(f"📦 成功打包后续测试集作为 Epoch 验证集: {val_names[0]} ~ {val_names[-1]}")
                ...
                print(f"⚠️ 后面没有测试集了，退化使用 {train_names[-1]} 的真实流量作为验证集。")
                dt_fallback, lt_fallback = read_one_type_da('tr_ts', train_names[-1], LABEL_DIR_NAME)
                val_dataset = TensorDataset(dt_fallback, lt_fallback)
                val_loader = DataLoader(val_dataset, batch_size=BATCH_SIZE*2, shuffle=False, num_workers=DATALOADER_NUM_WORKERS)
                val_loaders.append((train_names[-1], val_loader))
            ...
            for epoch in range(NUM_EPOCHS):
                ...
                for v_name, v_loader in val_loaders:
                    f_preds, f_trues, _, _, _, v_wmrd = run_evaluation(
                        model, v_loader, device, desc=f"Eval {v_name}"
                    )
                    current_epoch_wmrds.append(v_wmrd)
                    all_epoch_preds.append(f_preds)
                    all_epoch_trues.append(f_trues)
                
                epoch_mean_wmrd = np.mean(current_epoch_wmrds)
                
                with open(csv_file, 'a', newline='') as f:
                    writer = csv.writer(f)
                    writer.writerow([epoch + 1, train_loss, epoch_mean_wmrd])
                ...
                if epoch_mean_wmrd < best_mean_wmrd:
                    ...
                    if epochs_no_improve >= PATIENCE: 
                        print(f"   ↳ 🛑 连续 {PATIENCE} 个 Epoch 未改善，触发 Early Stopping，提前结束当前训练块。")
                        break

            print(f"✅ 联合训练块微调完成，最佳模型控制的平均 WMRD: {best_mean_wmrd:.4f}%")
            ...
            dataset = TensorDataset(dt, lt)
            
            test_dir = os.path.join(current_finetune_dir, "test_results", dataset_id)
            os.makedirs(test_dir, exist_ok=True)
            
            test_loader = DataLoader(dataset, batch_size=BATCH_SIZE*2, shuffle=False, num_workers=DATALOADER_NUM_WORKERS)
            final_preds, final_trues, val_mse, val_mae, val_r2, val_wmrd = run_evaluation(
                model, test_loader, device, desc=f"Testing {dataset_id}"
            )
            
            np.save(os.path.join(test_dir, 'preds.npy'), final_preds)
            np.save(os.path.join(test_dir, 'trues.npy'), final_trues)
            
            test_csv = os.path.join(test_dir, 'metrics.csv')
            with open(test_csv, 'w', newline='') as f:
                writer = csv.writer(f)
                writer.writerow(['val_mse', 'val_mae', 'val_r2', 'val_wmrd'])
                writer.writerow([val_mse, val_mae, val_r2, val_wmrd])
                
            print(f"📊 测试结果 [MSE: {val_mse:.4f} | MAE: {val_mae:.4f} | R2: {val_r2:.4f} | WMRD: {val_wmrd:.4f}%]")

...
def process_dat_file(file_name, i, k, real_dir, chazhi_dir):
    ...
    if i == 0 and k == 0:
        ...
        df.to_csv(real_dir + str(i) + '_' + str(k) + '.csv', index=False)
    ...
    target_freq_3 = np.arange(10001, 20001, 1000)
    
    target_freq_4 = np.arange(20001, 50001, 10000)
    
    target_freqs = np.concatenate((target_freq_1, target_freq_2, target_freq_3, target_freq_4))
...
real_dir = 'tr_ts/10_1e5_real'
chazhi_dir = 'tr_ts/10_1e5_chazhi'
make_dir(real_dir)
make_dir(chazhi_dir)
for i in range(10):
    for k in range(10):

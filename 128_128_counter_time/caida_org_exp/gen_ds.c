...
#define PACKET_SIZE       13       // 每个数据包13字节 [关键修改]
#define BUFFER_SIZE       4096     // 读取缓冲区
...
ProcessingResult process_dat(const char* path, uint32_t seed) {
    ...
    while (1) {
        ...
        // 处理每个数据包 [关键修改：直接使用全部13字节作为哈希输入]
        for (size_t i = 0; i < packets; ++i) {
            const uint8_t* pkt = aligned_ptr + i * PACKET_SIZE;
            // 使用全部13字节作为哈希输入 [关键修改]
            const uint32_t hash = MurmurHash3_x86_32(pkt, PACKET_SIZE, seed);

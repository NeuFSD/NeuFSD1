...
def main(comb_name, model_name, train_num, value1, value2):
    ...
    preds2 = np.load(f'{root}/{model_name}_10_1e5_results_1e-2/preds_{value2}.npy')
    ...
    def get_closest_to_avg_preds(preds):
        ...
        for idx, dataset_id in enumerate(test_ls):
            ...
            val_mrd_ls = []
            interp_ls = []
            # flited_val_freq = load_csv_analysis(dataset_id, 1000)
            # print(f'filted_count_results: {flited_val_freq}')
            # 加载真实值数据
            real_trues_1 = np.load(f'tr_ts/1_10_real/{dataset_id}.npy')
            real_trues_2 = np.load(f'tr_ts/10_1e5_real/{dataset_id}.npy')
            real_trues = np.vstack((real_trues_1, real_trues_2))
            
            # 获取当前数据集对应的预测值
            corr_preds = preds[idx*400 : (idx+1)*400]
            ...
            for sep_idx, sep_preds in enumerate(corr_preds):
                ...
                target_freqs = np.concatenate((
                    ...
                    np.arange(10001, 20001, 1000),
                    np.arange(20001, 50001, 10000)
                ))
                ...
                filted_interp_func = interp1d(filted_tgt_freqs, filted_sep_preds,
                                              kind='linear', fill_value='extrapolate')
                filted_new_target_freqs = np.arange(1001, 100001, 1)
